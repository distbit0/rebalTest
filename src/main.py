...
## TODO add support for any set of assets, not just btc and usd
...
def fetch_historical_data(start_date, end_date):
    ...
    for _ in range(num_requests):
        ...
        try:
            ...
            if not data:  # Handle cases where API returns no data for a chunk
                print(
                    f"Warning: No data returned from API for end date: {current_end_date}"
                )
            ...
            current_end_date -= timedelta(
                days=2001
            )  # Subtract slightly more than 2000 to avoid overlap, and to move backwards in time
            ...
            raise Exception(
                f"Failed to parse data, KeyError: {str(e)}. Response might not have 'Data.Data' structure."
            )
    ...
    filtered_data = [
        ...
        and datetime.fromtimestamp(d["time"])
        <= end_date  # Ensure all dates are within the requested range
    ]
...
def run_backtest(
    ...
    portfolio = {"btc_amount": initial_btc, "usd_amount": initial_usd_amt}
...
def compare_strategies(
    start_date, end_date, kelly_fraction, rebalance_freq=30, annualized_risk_free=0.0
):
    strategies = {
    ...
    for name, data in results.items():
        ...
        worst_period_return_pct = (
            min(strategy_returns[name]) * 100 if strategy_returns[name] else 0.0
        )
        ...
        comparison.append(
            (name, final_value, return_pct, worst_period_return_pct, max_drawdown_pct)
        )  # Removed avg_loss_pct
    ...
    for i in range(1, len(buy_and_hold_btc_values)):
        buy_hold_btc_returns.append(
            (buy_and_hold_btc_values[i] - buy_and_hold_btc_values[i - 1])
            / buy_and_hold_btc_values[i - 1]
        )

    buy_hold_worst_period_return_pct = (
        min(buy_hold_btc_returns) * 100 if buy_hold_btc_returns else 0.0
    )
    ...
    comparison.append(
        (
            "Buy & Hold BTC",
            buy_and_hold_btc_values[-1],
            btc_returns * 100,
            buy_hold_worst_period_return_pct,
            buy_hold_btc_max_drawdown_pct,
        )
    )  # Removed buy_hold_avg_loss_pct
    comparison.append(
        ("Buy & Hold USD", initial_usd, 0.0, 0.0, 0.0)
    )  # USD has zero risk
    ...
    for name, data in results.items():
        plt.plot(
            ...
            label=f"{name} (Fraction: {strategies[name]:.3f})",
        )  # Added fraction to label

    # Plot Buy & Hold BTC over time
    plt.plot(
        [d["date"] for d in price_data],
    ...
    print(
        f"  Annualized Risk-Free Rate (used in Kelly Calculation): {annualized_risk_free*100:.2f}%"
    )
    ...
    print(
        "  Worst Period Return:  Indicates the single worst percentage return experienced in any rebalance period during the backtest."
    )  # Updated explanation
    print(
        "  Max Drawdown:  Represents the largest percentage decline from a peak to a trough experienced during the backtest. It shows the maximum potential loss from a high point."
    )
    print(
        "  *Note: Lower Worst Period Return (more negative), and Max Drawdown generally indicate higher risk.*"
    )  # Updated note
...
if __name__ == "__main__":
    start_date = datetime(2014, 1, 1)  # Example: Extended start date
    end_date = datetime(2025, 1, 1)
    rebalance_freq = 10  # Monthly rebalancing
    ...
    try:
        ...
        if not data:
            ...
            print(
                f"Calculated Optimal Kelly fraction (with risk-free rate): {kelly_fraction:.4f}"
            )
            ...
            optimal_row = comparison_df[
                comparison_df["Strategy"] == "Optimal Kelly"
            ].iloc[0]
            half_row = comparison_df[comparison_df["Strategy"] == "Half Kelly"].iloc[0]

            if (
                optimal_row["Return %"] >= half_row["Return %"]
            ):  # Optimal Kelly should ideally have >= return than Half Kelly
                print(
                ...
                print(
                    "- Look-ahead bias in parameter estimation (still present as discussed)"
                )
